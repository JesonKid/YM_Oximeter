...
* @param Port--端口
* @param Pin--引脚
* @param BitVal--输出状态 Bit_RESET--输出低电平 Bit_SET--输出高电平
*/

void GPIO_WriteBit(GPIO_PORT_ENUM_E Port, GPIO_PIN_ENUM_E Pin, BitAction BitVal)
{
   if(BitVal == Bit_RESET)GPIO_ClearBits(Port,Pin);
   else if(BitVal == Bit_SET)GPIO_SetBits(Port,Pin);
}
...
void GPIO_20mA_Mode(GPIO_PORT_ENUM_E Port, GPIO_PIN_ENUM_E Pin, FunctionalState En)
{
    ...
    {
    case GPIOPortA:
        if (Pin == GPIOPin0)pConfig->sw_ma30_pa00_en = En;
        else if (Pin == GPIOPin1)pConfig->sw_ma30_pa01_en = En;
        break;
    case GPIOPortB:
        if (Pin == GPIOPin0)pConfig->sw_ma30_pb00_en = En;
        else if (Pin == GPIOPin1)pConfig->sw_ma30_pb01_en = En;
        else if (Pin == GPIOPin2)pConfig->sw_ma30_pb02_en = En;
        else if (Pin == GPIOPin3)pConfig->sw_ma30_pb03_en = En;
        break;
    default:
        break;
    }
