...
void fic_IRQHandler(void)
{
    ...
    else if (0 == sCellSpan)
    {
        ...
        if (74 == sSpo2Span)
        {
            ...
            ANAREG_1->reg_adsar_mux_in_f.adc_ext_input_p = SARADC_AIN3_PA03; // VBAT Sample pin，SARADC切换至PA04，电池电量检测
        }
        ...
        else if (76 < sSpo2Span && 84 >= sSpo2Span)
        {
            ...
            if (84 == sSpo2Span)
            {
                ...
                if (0 > gCellVolt)
                {
                    GPIO_ClearBits(GPIOPortA, GPIOPin2);   // power off，电池电量低于阈值，系统断电
                }
                //Reset and switch to normal sample
                gSampVolt = 0;

                //Restore normal spo2 sample
                ANAREG_1->reg_adsar_mux_in_f.adc_ext_input_p = SARADC_AIN2_PA02;    //SARADC切换至PA03，spo2检测
            }
        }
    }
